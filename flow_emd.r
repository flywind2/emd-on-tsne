...
#  tSNEcolumns <- grep( "tSNE.", colnames(fileColumnsFrame))
  tSNEcolumns <- c(grep( column1_name, colnames(fileColumnsFrame)), grep( column2_name, colnames(fileColumnsFrame)))
  
  print( sprintf("Using data columns %s(#%d), %s(#%d)", column1_name, tSNEcolumns[1], column2_name, tSNEcolumns[2])) 
...
#    fillMatrix[ fromFrame$tSNE1[selected_row], fromFrame$tSNE2[selected_row] ] <- fillMatrix[ fromFrame$tSNE1[selected_row], fromFrame$tSNE2[selected_row] ] + 1
    fillMatrix[ fromFrame[column1_name][selected_row,], fromFrame[column2_name][selected_row,] ] <- fillMatrix[ fromFrame[column1_name][selected_row,], fromFrame[column2_name][selected_row,] ] + 1
  }
...
plot_binned_frame <- function( binnedFrame, my_color, my_ylim, my_xlim, do_add ) {
  ...
  if( !do_add ) {
    title(xlab="bin number", ylab="bin number")
  } else {
    grid( my_xlim[2], my_ylim[2], lwd=1, col="gray")
  }
...
graph_binned_plots <- function (matrix1, 
  ...
  png(output_graphic_file, width = 1200, height = 1200)
  par(mfrow=c(2,2))

  smoothScatter(raw_tsne1_frame, colramp = colorRampPalette(c("white", "steelblue2", "yellow", "orange", "red")))
  smoothScatter(raw_tsne2_frame, colramp = colorRampPalette(c("white", "green", "yellow", "orange", "red")))
  
  my_ylim <- c(1,ncol(matrix2))
  my_xlim <-  c(1, nrow(matrix2))
  
  plot_binned_frame( create_frame_from_matrix(matrix2), "green", my_ylim, my_xlim, FALSE)
#  par(new=T)
  plot_binned_frame( create_frame_from_matrix(matrix1), "steelblue2", my_ylim, my_xlim, TRUE)
  ...
  plot_binned_frame( create_frame_from_matrix(matrix1), "steelblue2", my_ylim, my_xlim, FALSE)
#  par(new=T)
  plot_binned_frame( create_frame_from_matrix(matrix2), "green", my_ylim, my_xlim, TRUE)
...
execute_emd <- function( sample1_file, sample2_file, output_graphic_file = NULL) {
  ...
  print( sprintf("matrix dimension is %d", as.integer(matrix_size) ))
